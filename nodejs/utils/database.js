...
const DB_PATH = process.env.DB_PATH || path.join(__dirname, '..', 'birdchat.db');
...
export function logSecurityEvent(type, event, ip = null, uid = null, userAgent = '') {
  const insertLog = db.prepare(`
    INSERT INTO security_logs (event_type, event_description, ip_address, user_id, user_agent)
    VALUES (?, ?, ?, ?, ?)
  `);

  insertLog.run(type, event, ip || 'UNKNOWN', uid || null, userAgent || 'NodeJS');
}
